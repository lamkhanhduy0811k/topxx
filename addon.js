...
const manifest = {
    ...
    version: '10.0.0',
    name: 'NGUONC Cinema',
    description: 'Xem phim từ NGUONC API - Bản chính thức',
...
async function fetchCatalog(catalogId, page = 1) {
    try {
        let url = '';
        
        if (catalogId === 'phim-moi-cap-nhat') {
            url = `${API_BASE}/films/phim-moi-cap-nhat?page=${page}`;
        } else if (catalogId === 'phim-bo') {
            url = `${API_BASE}/films/danh-sach/phim-bo?page=${page}`;
        } else if (catalogId === 'phim-le') {
            url = `${API_BASE}/films/danh-sach/phim-le?page=${page}`;
        }
...
builder.defineCatalogHandler(async (args) => {
    ...
    const data = await fetchCatalog(args.id, page);
...
console.log('✅ NGUONC Cinema v10.0.0 đang chạy!');
